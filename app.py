...
def get_text_chunks(text):
    ...
    return chunks  # Return the split text chunks

def get_vector_store(text_chunks):
    embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
    text = ' '.join(text_chunks)  # Concatenate text chunks into a single string
    vector_store = FAISS.from_texts([text], embedding=embeddings)  # Pass a list of text
...
def user_input(user_question):
    ...
    # Construct input dictionary with input_documents and question keys
    input_data = {"input_documents": docs, "question": user_question}

    response = chain(input_data, return_only_outputs=True)
    
    st.write("Reply: ", response["output_text"])  # Display response using Streamlit write method
